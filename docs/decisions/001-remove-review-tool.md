...
- **Add a generic `claude` raw-passthrough tool.** Cleaner fit than a reshaped `review` if a generic surface is wanted, but no audience signal justifies adding one today.
...
- **Documentation**: README adds a "Code review with this CLI" section with the hardened invocation plus a Claude Code skill template. The template is documentation, not code.
- **Bounded loss**: remote MCP clients without shell access (Claude Desktop, etc.) lose the bridge-mediated path to claude-as-reviewer. Mitigated by the host's primary chat surface, which already provides native Claude access on those hosts.
