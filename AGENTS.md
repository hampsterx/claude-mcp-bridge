...
- **Native structured output** uses Claude CLI's `--json-schema` flag (no Ajv dependency). Do not swap in Ajv to "match" the codex bridge; the native path is stricter and faster here.
...
- **Broader secret redaction patterns** (base64, API keys, Bearer tokens) are a feature. Do not narrow; this bridge is deliberately stricter than the codex one.
